...
export const useTabStore = defineStore({
  ...
  actions: {
    ...
    async removeTab(tab: RouteLocationNormalized, router: Router, goPath?: string) {
    ...
    async removeOtherTabs(route: RouteLocationNormalized, router: Router) {
      ...
      for (const fullPath of closePathList) {
        if (fullPath !== route.fullPath) {
          const closeItem = this.tabList.find((item) => item.fullPath === fullPath)
          if (!closeItem) {
            continue
          }
