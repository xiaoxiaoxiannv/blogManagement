        ...
        >
          <template #label>
            <TabContent
              ...
              :is-active="tab.fullPath === getActive ? true : false"
            ></TabContent>
          </template>
        </ElTabPane>
      </template>
...
const getDomHeight = computed((): CSSProperties => {
  return {
    height: `${unref(getTabsHeight) / 16}rem`
  }
})
...
onMounted(() => {
  ...
  if (elTabs) {
    elTabs.style.setProperty('--el-tabs-header-height', `${unref(getTabsHeight) / 16}rem`)
  }
})
...
listenerRouteChange((route) => {
  ...
  // tabStore.addTab(unref({...route,name:route.fullPath}))
  // 判断是否已存在
  const tabExisted = tabStore.getTabList.find(tab => tab.name === route.name)
  if (tabExisted) {
    tabStore.removeTab(tabExisted, router)
  }
